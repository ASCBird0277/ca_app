...
(() => {
  ...
  function initializeMap() {
    ...
    // Reference overlays for boundaries and places (states, cities; counties at closer zoom levels)
    const boundariesPlaces = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}', {
    ...
    const transportation = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}', {
      attribution: 'Esri Reference — Transportation',
      maxZoom: 19,
    });

    // Default to Satellite view; user can switch to Streets
    ...
    const overlays = {
      'Boundaries & Places': boundariesPlaces,
      'Roads & Highways': transportation,
    };
    boundariesPlaces.addTo(state.map);
    transportation.addTo(state.map);
    L.control.layers(baseLayers, overlays, { position: 'topright' }).addTo(state.map);
  ...
  function fitMapToVisibleProperties() {
    if (!state.map) return;
    const coords = state.visibleProperties
      .filter((p) => p && p.hasCoordinates && typeof p.latitude === 'number' && typeof p.longitude === 'number')
  ...
  function bindUI() {
    ...
    // Advanced Search toggle behavior (keep panel open while interacting)
    const advToggle = document.getElementById(SELECTORS.advancedSearchToggle);
    const advPanel = document.getElementById(SELECTORS.advancedFilters);
    if (advToggle && advPanel) {
      const headerEl = advPanel.closest('header');
      advToggle.addEventListener('click', (e) => {
        e.stopPropagation();
        advPanel.classList.toggle('hidden');
      });
      // Only hide when clicking outside the header entirely
      document.addEventListener('click', (event) => {
      ...
      document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') advPanel.classList.add('hidden');
      });
    }
  ...
  async function onSearchKeydown(event) {
    ...
    if (!unlocked) {
      ...
      await performSearch();
      try {
        // If exactly one property matches, or exact name match exists, open it
        const lower = candidate.toLowerCase();
        const exactMatches = (state.visibleProperties || []).filter((p) => String(p.property || '').toLowerCase() === lower);
        let target = null;
        if (exactMatches.length === 1) {
          target = exactMatches[0];
        } else if ((state.visibleProperties || []).length === 1) {
          target = state.visibleProperties[0];
        }
        if (target && target.propertyId) {
          centerOnProperty(target.propertyId);
          openStaffDrawer(target.propertyId, target.property || candidate);
        }
      } catch (_) { /* no-op */ }
    }
  }
  ...
  function updateRegions() {
    ...
    panel.innerHTML = '';
    if (!regions.length) {
      panel.innerHTML = '<p class="text-xs text-slate-600">No regions available.</p>';
      return;
    }
  ...
  function renderSearchOverlay() {
    ...
    if (!query || !state.showSearchOverlay) {
      hideSearchOverlay();
      return;
    }
    ...
    if (employeeMatches.length) {
      ...
      employeeMatches.forEach((match) => {
        ...
        // Clicking a staff result sets the query and runs search.
        // If exactly one property remains visible after the search,
        // auto-center, open popup, and show staff drawer.
        item.addEventListener('click', async () => {
          try {
            const input = getElement(SELECTORS.searchInput);
            const value = primary.textContent || '';
            input.value = value;
            state.filters.query = value;
            state.showSearchOverlay = false;
            hideSearchOverlay();
            await performSearch();
            if (Array.isArray(state.visibleProperties) && state.visibleProperties.length === 1) {
              const only = state.visibleProperties[0];
              if (only && only.propertyId) {
                centerOnProperty(only.propertyId);
                openStaffDrawer(only.propertyId, only.property || 'Staff');
              }
            }
          } catch (_) {}
        });
    ...
    if (propertyMatches.length) {
      ...
      propertyMatches.forEach((property) => {
        ...
        item.append(left, right);
        // Clicking a property result sets the query, runs search,
        // centers map, opens popup, and shows staff details
        item.addEventListener('click', async () => {
          try {
            const input = getElement(SELECTORS.searchInput);
            const value = primary.textContent || '';
            ...
            if (propertyId) {
              centerOnProperty(propertyId);
              openStaffDrawer(propertyId, property.property || value);
            }
          } catch (_) {}
        });
        list.appendChild(item);
  ...
  function createPropertyCard(property) {
    ...
    card.className = 'property-card rounded-lg border border-slate-800 bg-slate-900/60 p-4 shadow-md transition';
    card.setAttribute('data-property-id', property.propertyId);

    const vacancyClass = property.hasVacancy ? 'bg-amber-300/90 text-black' : '';
    const vacancyLabel = property.hasVacancy ? 'Vacancy' : '';
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        ...
        ${vacancyLabel ? `<span class=\"status-chip ${vacancyClass}\">${vacancyLabel}</span>` : ''}
      </div>
  ...
  async function openStaffDrawer(propertyId, propertyName) {
    ...
    content.innerHTML = '<p class="text-sm text-slate-600">Loading...</p>';
  ...
  function renderDrawerContent(data) {
    ...
    employees.sort((a, b) => {
      ...
      const pb = priorityFor(b.jobTitle);
      if (pa !== pb) return pa - pb;
      return String(a.employeeName || '').localeCompare(String(b.employeeName || ''));
    });
    if (!employees.length) {
      content.innerHTML = '<p class="text-sm text-slate-700">No positions recorded for this property.</p>';
      return;
    }
    ...
    employees.forEach((employee) => {
      ...
      const alsoWorksAt = Array.isArray(employee.otherProperties) && employee.otherProperties.length
        ...
        : '';
      const borderClass = employee.isVacant ? 'border-yellow-400' : 'border-slate-200';
      const isMatch = !!q && String(name).toLowerCase().includes(q);
      const classes = [
        'rounded-lg',
        borderClass,
        'border',
      ...
      if (isMatch) classes.push('staff-highlight');
      card.className = classes.join(' ');
      card.innerHTML = `
        <div class="flex items-start justify-between">
          <div class="min-w-0">
            <p class="text-sm font-semibold">${jobTitle}</p>
            <p class="text-xs text-slate-600 truncate">${name}</p>
          </div>
          
        </div>
        <div class="mt-2 space-y-1 text-xs text-slate-600">
          ${employee.email ? `<p>Email: <a href="mailto:${employee.email}" class="text-slate-900 underline">${employee.email}</a></p>` : ''}
          ${employee.phone ? `<p>Phone: <a href="tel:${employee.phone}" class="text-slate-900 underline">${employee.phone}</a></p>` : ''}
        </div>
        ${alsoWorksAt ? `<div class="mt-2 flex justify-end text-[11px] text-slate-600">${alsoWorksAt}</div>` : ''}
      `;
